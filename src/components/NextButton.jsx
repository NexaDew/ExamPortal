...
import {ArrowBigRightDash,Bookmark} from 'lucide-react'
...
const NextButton = ({getQuestion,index}) => {
  ...
  return (
    <div className='flex justify-between mr-20 mb-20'>
          <button className=' flex bg-white font-bold w-60 h-15 ml-20 rounded-lg items-center pl-7' onClick={mark}>Mark for review<Bookmark /></button>
          <button className=' flex bg-black text-white w-60 h-15 rounded-lg items-center pl-7' onClick={nextQuestion}>Next Question<ArrowBigRightDash /></button>
    </div>
  )
}
